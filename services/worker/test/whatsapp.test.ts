...
function model(over: Partial<ReportModel> = {}): ReportModel {
  return {
    ...
    headline: { sentence: 'August 2026 worked — USD 7,810.07 of contribution after ad spend, up on last month.', contributionMinor: 781_007, direction: 'up' },
    efficiency: { merValue: 2.82, breakEvenMer: 1.8, targetMer: null, blendedCacMinor: 5842, firstOrderContributionMinor: 5431 },
    margin: { rows: [{ label: 'Net sales', amountMinor: 1_860_000, kind: 'subtotal' }, { label: 'Contribution after ad spend', amountMinor: 781_007, kind: 'total' }], completeness: 1 },
    channels: [],
    whatChanged: [],
    findings: [
      { title: 'Payback broken', entityLabel: 'Whole account', family: 'waste', valueLabel: 'USD 558.96 at stake', text: '…' },
      { title: 'Spend headroom', entityLabel: 'Whole account', family: 'growth', valueLabel: '+USD 4,508.01 opportunity', text: '…' },
      { title: 'Claim gap', entityLabel: 'Google Ads', family: 'measurement', valueLabel: 'no money at stake', text: '…' },
    ],
...
describe('WhatsApp summary (task 5.B6)', () => {
  it('is a short block: headline, three numbers, the top finding', () => {
    ...
    expect(lines[2]).toBe('MER 2.82 (break-even 1.80) · Net sales USD 18,600.00 · Contribution USD 7,810.07');
  ...
  it('leads with a growth finding when there is no waste finding', () => {
    const text = buildWhatsAppSummary(
      model({ findings: [{ title: 'Spend headroom', entityLabel: 'Whole account', family: 'growth', valueLabel: '+USD 4,508.01 opportunity', text: '…' }] }),
    );
  ...
  it('says nothing-needs-changing when there is no actionable finding', () => {
    const text = buildWhatsAppSummary(
      model({
        ...
        findings: [{ title: 'Claim gap', entityLabel: 'Google Ads', family: 'measurement', valueLabel: 'no money at stake', text: '…' }],
      }),
    );

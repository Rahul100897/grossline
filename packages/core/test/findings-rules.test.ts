...
function fired(outcomes: RuleOutcome[]) {
  return outcomes
    .filter((o) => o.status === 'fired')
    .map((o) => (o as { finding: unknown }).finding);
}
...
describe('rule: below break-even MER', () => {
  ...
  it('is ok when MER is above break-even', () => {
    expect(belowBreakEvenMer.run(base({ account: { ...base().account, merValue: 2.5 } }))).toEqual([
      { status: 'ok' },
    ]);
  });
  it('skips without margin data', () => {
    const out = belowBreakEvenMer.run(
      base({
...
describe('rule: dead campaign', () => {
  ...
  it('fires full spend for a campaign over the floor with zero orders', () => {
    ...
    expect(f).toHaveLength(1);
    expect(f[0]).toMatchObject({
      entityKey: 'campaign:google_ads:2',
      moneyImpactMinor: 60_000,
      severity: 'attention',
    });
  });
  it('is ok when every campaign has orders', () => {
    const ok = deadCampaign.run(
      base({ campaigns: campaigns.map((c) => ({ ...c, attributedOrders: 5 })) }),
    );
  ...
  it('skips without per-campaign attribution', () => {
    expect(
      deadCampaign.run(
        base({ availability: { ...base().availability, hasCampaignAttribution: false } }),
      )[0]!.status,
    ).toBe('skipped');
  });
});
...
describe('rule: branded search share', () => {
  ...
  it('fires branded spend when share exceeds the ceiling', () => {
    ...
    expect(f[0]).toMatchObject({
      moneyImpactMinor: 400_000,
      currentValue: 0.4,
      comparisonValue: 0.35,
    });
  });
  ...
  it('skips when branded keywords are not identifiable', () => {
    expect(
      brandedSearchShare.run(
        base({ availability: { ...base().availability, hasBrandedClassification: false } }),
      )[0]!.status,
    ).toBe('skipped');
  });
});
...
describe('rule: search term waste', () => {
  ...
  it('skips without a search-term report', () => {
    expect(
      searchTermWaste.run(
        base({ availability: { ...base().availability, hasSearchTerms: false } }),
      )[0]!.status,
    ).toBe('skipped');
  });
});
...
describe('rule: discount leakage', () => {
  it('fires the absolute rise when discount share climbs beyond tolerance', () => {
    ...
    const out = discountLeakage.run(
      base({ account: { ...base().account, discountsMinor: 90_000 }, priorDiscountShare: 0.05 }),
    );
    const f = fired(out);
    expect(f[0]).toMatchObject({
      moneyImpactMinor: 40_000,
      currentValue: 0.09,
      comparisonValue: 0.05,
    });
  });
  it('is ok within tolerance', () => {
    expect(
      discountLeakage.run(
        base({ account: { ...base().account, discountsMinor: 70_000 }, priorDiscountShare: 0.05 }),
      ),
    ).toEqual([{ status: 'ok' }]);
  });
...
describe('rule: refund outlier', () => {
  const products = [
    {
      ...
      label: 'Serum',
      refundRate: 0.15,
      refundedValueMinor: 30_000,
      attributedSpendMinor: 20_000,
    },
    {
      key: 'P2',
      label: 'Cream',
      refundRate: 0.08,
      refundedValueMinor: 5_000,
      attributedSpendMinor: 10_000,
    },
  ];
  ...
  it('skips without per-product refund data', () => {
    expect(
      refundOutlier.run(
        base({ availability: { ...base().availability, hasProductRefunds: false } }),
      )[0]!.status,
    ).toBe('skipped');
  });
});
...
describe('rule: payback broken', () => {
  it('fires CAC gap × new customers', () => {
    ...
    expect(fired(out)[0]).toMatchObject({
      moneyImpactMinor: 300_000,
      currentValue: 8_000,
      comparisonValue: 5_000,
    });
  });
  it('is ok when acquisition pays back on the first order', () => {
    expect(
      paybackBroken.run(base({ account: { ...base().account, blendedCacMinor: 4_000 } })),
    ).toEqual([{ status: 'ok' }]);
  });
  it('skips without margin (no first-order contribution)', () => {
    expect(
      paybackBroken.run(
        base({
          availability: { ...base().availability, hasMargin: false },
          account: { ...base().account, firstOrderContributionMinor: null },
        }),
      )[0]!.status,
    ).toBe('skipped');
  });
...
describe('rule: spend pacing', () => {
  it('fires the overspend above target', () => {
    ...
    const out = spendPacing.run(
      base({ account: { ...base().account, spendProjectedMonthEndMinor: 620_000 } }),
    );
    expect(fired(out)[0]).toMatchObject({
      moneyImpactMinor: 120_000,
      currentValue: 620_000,
      comparisonValue: 500_000,
    });
  });
  it('is ok within the pacing ceiling', () => {
    expect(
      spendPacing.run(
        base({ account: { ...base().account, spendProjectedMonthEndMinor: 540_000 } }),
      ),
    ).toEqual([{ status: 'ok' }]);
  });
  it('skips without a spend target', () => {
    expect(
      spendPacing.run(
        base({
          availability: { ...base().availability, hasSpendTarget: false },
...
describe('no rule fires on incomplete data', () => {
  it('every rule skips when all inputs are absent', () => {
    ...
    for (const rule of RULES) {
      ...
      expect(
        out.every((o) => o.status === 'skipped'),
        `${rule.id} must skip on absent data`,
      ).toBe(true);
      expect(
        out.some((o) => o.status === 'fired'),
        `${rule.id} must not fire on absent data`,
      ).toBe(false);
    }
  });
});

...
**Current phase: Phase 5 — Report delivery.** Phases 0–4 are complete. See `docs/phase-2-handover.md` for the metric layer (every metric in `docs/metrics.md` computes, golden-tested, reconciliation-agreed), `docs/phase-1-handover.md` for connector state (one real dev store connected; Meta/Google still on fixtures), `docs/phase-3-handover.md` for the admin console (overview, merchants, issues, metrics explorer, billing + invoice PDFs, support inbox, settings, reconciliation panel), and `docs/phase-4-handover.md` for the findings engine (per-tenant calibration, nine golden-tested rules, ranking/suppression, review console, two-tier commentary with the model-figure guard, recommendation tracking, findings-as-issues). Phase 5 wraps the approved findings in the monthly PDF/email — the HTML→PDF plumbing already exists (Phase 3 invoice PDFs). The Phase 5 spec is not written yet; do not start report work without it. Do not build ahead of the current phase.
